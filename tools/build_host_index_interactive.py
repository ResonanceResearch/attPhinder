...
def interactive_prompt_args() -> List[str]:
    ...
    flank = ask("Flank around indexed host features in bp", "5000")

    print()
    print("Global broader-search chunks allow the webapp to search beyond indexed feature neighborhoods,")
    ...
    serine_hotspots_answer = ask("Include serine-integrase CDS hotspots such as groL/groEL? yes/no", "yes").lower()
    if serine_hotspots_answer in {"n", "no", "false", "0"}:
        argv.append("--no-serine-cds-hotspots")
    ...
    gff_path = ask("Optional GFF/GFF3 path for feature annotations; leave blank if not needed", "")
    if gff_path:
