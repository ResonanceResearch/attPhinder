...
- tRNA/tmRNA/rRNA neighborhood sequences
...
def build_neighborhoods(records: List[Dict], feature_types: set, flank: int, max_neighborhood_bp: int) -> List[Dict]:
    neighborhoods = []
    feature_counter = 0
    ...
    for rec in records:
        ...
        for feat in rec.get("features", []):
            if feat.get("type") not in feature_types:
                continue
            loc = feat["location"]
            start = int(loc["start"])
            end = int(loc["end"])
...
def interactive_prompt_args() -> List[str]:
    """
    Ask for inputs interactively when the script is run without command-line arguments.

    This is useful when running from Visual Studio Code using the Run button.
    """
    print()
    print("Interactive host_index.json builder")
    print("-----------------------------------")
    ...
    input_path = ask("Path to bacterial GenBank or FASTA file")
    while not input_path:
        print("Input file is required.")
        input_path = ask("Path to bacterial GenBank or FASTA file")

    # Allow drag-and-drop paths with quotes on macOS/Windows.
    input_path = input_path.strip().strip('"').strip("'")

    default_out = os.path.join(os.path.dirname(os.path.abspath(input_path)) or ".", "host_index.json")
    out_path = ask("Output JSON path", default_out).strip().strip('"').strip("'")

    flank = ask("Flank around tRNA/tmRNA/rRNA features in bp", "5000")

    print()
    print("Global broader-search chunks allow the webapp to search beyond tRNA neighborhoods,")
    print("but they can make host_index.json much larger.")
    include_global_answer = ask("Include broader whole-genome chunks? yes/no", "no").lower()
    include_global = include_global_answer in {"y", "yes", "true", "1"}

    argv = [input_path, "--out", out_path, "--flank", flank]

    if include_global:
        chunk_size = ask("Global chunk size", "20000")
        chunk_step = ask("Global chunk step", "10000")
        argv += ["--chunk-size", chunk_size, "--chunk-step", chunk_step]

        omit_seq_answer = ask("Omit global chunk sequences to keep JSON smaller? yes/no", "no").lower()
        if omit_seq_answer in {"y", "yes", "true", "1"}:
            argv.append("--omit-global-sequences")
    else:
        argv.append("--no-global-chunks")

    gff_path = ask("Optional GFF/GFF3 path for tRNA annotations; leave blank if not needed", "")
    if gff_path:
        argv += ["--gff", gff_path.strip().strip('"').strip("'")]

    pretty_answer = ask("Pretty-print JSON? yes/no", "no").lower()
    if pretty_answer in {"y", "yes", "true", "1"}:
        argv.append("--pretty")

    print()
    print("Running with:")
    print("python build_host_index_interactive.py " + " ".join(f'"{x}"' if " " in x else x for x in argv))
    print()
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--gff", default=None, help="Optional GFF/GFF3 file with tRNA/tmRNA/rRNA features, useful with FASTA input.")
    parser.add_argument("--out", default="host_index.json", help="Output JSON file. Default: host_index.json")
    parser.add_argument("--flank", type=int, default=5000, help="Flank around tRNA/tmRNA/rRNA features. Default: 5000 bp.")
    ...
    if input_format == "genbank":
        records, gb_meta = parse_genbank(args.input, wanted_feature_types)
    ...
    if args.gff:
        gff_features = parse_gff_features(args.gff, wanted_feature_types)
        by_contig = {}
    ...
    if len(neighborhoods) == 0:
        print()
        print("WARNING: No tRNA/tmRNA/rRNA neighborhoods were found.")
        print("If you used FASTA input, provide a GFF file with --gff.")
        print("If you used GenBank input, check whether the feature types are annotated as tRNA/tmRNA/rRNA.")

    if size_mb > 50:
        print()
        print("WARNING: host_index.json is large.")
        print("Consider using --no-global-chunks or --omit-global-sequences for a smaller file.")
        print("For first-pass attB prediction, tRNA neighborhoods alone are often the most important.")
...
if __name__ == "__main__":
    try:
        if len(sys.argv) == 1:
            prompted_args = interactive_prompt_args()
            raise SystemExit(main(prompted_args))
        else:
