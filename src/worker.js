...
async function readFiles(fileList, role) {
  ...
  for (const file of fileList) {
    ...
    const recs = isGB ? parseGenBank(text, file.name, role) : parseFasta(text, file.name);
...
async function run(payload) {
  ...
  postLog('Parsing bacterial files in host-optimized mode: sequence plus tRNA/tmRNA features only…');
  ...
  for (const p of phages) {
    ...
    for (const b of bacteria) {
      postLog(`Comparing ${p.name} to ${b.name}: ${fmt(b.sequence.length)} bp, ${findTRNAs(b).length} parsed tRNA/tmRNA feature(s).`);
