...
function scoreHit(core, opts) {
  ...
  const isFeatureNeighborhood = ['tRNA', 'indexed_feature'].includes(opts.searchType);
...
function analyzePair(phage, bacterium, settings) {
  ...
  for (const queryRegion of phageSearchRegions) {
    ...
    for (const trna of trnas) {
      ...
      evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: 'tRNA', product: trna.product, settings, queryMode: mode });
    }
...
function betterDuplicateRepresentative(candidate, incumbent) {
  ...
  const priority = { tRNA: 4, indexed_feature: 3, indexed_global: 2, global: 1 };
