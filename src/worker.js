...
function parseGenBank(text, fileName, role) {
  const records = [];
  const keepTypes = role === 'host' ? new Set(['tRNA', 'tmRNA', 'rRNA']) : new Set(['CDS', 'tRNA', 'tmRNA', 'rRNA']);
...
function findIntegrases(record) {
  ...
  */
  ...
  return record.features.filter(f => f.type === 'CDS' && f.start && f.end).map(f => {
    ...
    return { feature: f, text, score, flags };
...
function findHostIntegrationFeatures(record) {
  return record.features.filter(f => /^(tRNA|tmRNA|rRNA)$/i.test(f.type) && f.start && f.end).map((f, idx) => {
    const product = f.qualifiers?.product || f.qualifiers?.gene || f.qualifiers?.note || `${f.type}_${idx+1}`;
    return {
      feature: f,
      featureType: f.type,
      product: Array.isArray(product) ? product.join('; ') : product,
...
function nearestIndexedFeature(target, hostCoord, bacterium) {
  ...
  if (!indexed.length && Array.isArray(bacterium.features)) {
    for (const f of bacterium.features) {
      if (!/^(tRNA|tmRNA|rRNA)$/i.test(f.type || '') || !f.start || !f.end) continue;
      candidates.push({
        type: f.type,
...
function scoreHit(core, opts) {
  ...
  const isFeatureNeighborhood = ['tRNA', 'rRNA', 'indexed_feature'].includes(opts.searchType);
  const trnaScore = isFeatureNeighborhood ? 10 : 0;
...
function analyzePair(phage, bacterium, settings) {
  ...
  const integrationFeatures = usingHostIndex ? [] : findHostIntegrationFeatures(bacterium);
  if (usingHostIndex) {
    ...
    if (!integrationFeatures.length && bacterium.format === 'GenBank') postLog(`No tRNA/tmRNA/rRNA features found in ${bacterium.name}; global search only.`);
  ...
  for (const queryRegion of phageSearchRegions) {
    ...
    if (usingHostIndex) {
      ...
      for (const target of targets) {
        evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: 'indexed_feature', product: target.product || '', settings, queryMode: mode });
      }
      if (settings.globalSearch) {
        ...
        if (!chunks.length) {
          ...
          for (const { target, support } of ranked) {
            ...
            evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target: t, searchType: 'indexed_global', product: '', settings, queryMode: mode });
          }
        }
      }
      continue;
    ...
    postLog(`Testing ${integrationFeatures.length} tRNA/tmRNA/rRNA region(s) for ${bacterium.name} against ${phage.name}.`);
    for (const hostFeature of integrationFeatures) {
      ...
      target.featureMeta = { type: hostFeature.featureType, label: hostFeature.product, start: tF.start, end: tF.end, strand: tF.strand || '', contigId: bacterium.name || '' };
      target.contigLength = bacterium.sequence.length;
      const searchKind = hostFeature.featureType === 'rRNA' ? 'rRNA' : 'tRNA';
      evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: searchKind, product: hostFeature.product, settings, queryMode: mode });
    }
    ...
    if (settings.globalSearch) {
      ...
      for (const region of globalRegions) {
        ...
        evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: 'global', product: '', settings, queryMode: mode });
      }
    }
  }
  return outputs;
...
function evaluateTarget(ctx) {
  const { outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType, product, settings, queryMode } = ctx;
  ...
  const featureType = nearestFeature?.type || '';
  const featureLabel = nearestFeature?.label || '';
...
function betterDuplicateRepresentative(candidate, incumbent) {
  ...
  const priority = { tRNA: 5, rRNA: 4, indexed_feature: 3, indexed_global: 2, global: 1 };
...
async function run(payload) {
  ...
  for (const p of phages) {
    ...
    for (const b of bacteria) {
      ...
      else postLog(`Comparing ${p.name} to ${b.name}: ${fmt(b.sequence.length)} bp, ${findHostIntegrationFeatures(b).length} parsed tRNA/tmRNA/rRNA feature(s).`);
