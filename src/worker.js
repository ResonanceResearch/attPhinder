...
function scoreHit(core, opts) {
  ...
  const canonicalFeature = ['tRNA', 'tmRNA', 'rRNA'].includes(featureClass) || ['tRNA', 'tmRNA', 'rRNA', 'indexed_feature'].includes(opts.searchType);
  ...
  const familyScore = integraseFamily === 'serine' && featureClass.startsWith('CDS_hotspot') ? 8 : 0;
  ...
  if (model === 'serine_stem_core') {
    ...
    return Math.max(0, Math.min(100, Math.round(coreScore + stemScore + seedScore + featureScore + familyScore + repeatPenalty)));
  }

  const coreScore = Math.min(70, core.len * 2.2);
  return Math.max(0, Math.min(100, Math.round(coreScore + seedScore + featureScore + familyScore + repeatPenalty)));
}
...
function analyzePair(phage, bacterium, settings) {
  ...
  for (const queryRegion of phageSearchRegions) {
    const { intF, intName, pWin, mode, integraseFamily } = queryRegion;
    ...
    if (usingHostIndex) {
      ...
      for (const target of targets) {
        evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: 'indexed_feature', product: target.product || '', settings, queryMode: mode, integraseFamily });
      }
      if (settings.globalSearch) {
        ...
        if (!chunks.length) {
          ...
          for (const { target, support, smallSupport } of ranked) {
            ...
            evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target: t, searchType: 'indexed_global', product: '', settings, queryMode: mode, integraseFamily });
          }
        }
      }
      continue;
    ...
    postLog(`Testing ${integrationFeatures.length} attB-priority feature region(s) for ${bacterium.name} against ${phage.name}.`);
    for (const hostFeature of integrationFeatures) {
      ...
      evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: searchKind, product: hostFeature.product, settings, queryMode: mode, integraseFamily });
    }
    ...
    if (settings.globalSearch) {
      ...
      for (const region of globalRegions) {
        ...
        evaluateTarget({ outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType: 'global', product: '', settings, queryMode: mode, integraseFamily });
      }
    }
  }
  return outputs;
...
function evaluateTarget(ctx) {
  const { outputs, phage, bacterium, intF, intName, pWin, pSeqPlus, pSeqMinus, pPlusIndex, pMinusIndex, target, searchType, product, settings, queryMode, integraseFamily } = ctx;
  ...
  const consider = (core, t) => {
    ...
    const score = scoreHit(core, { searchType, hostFeatureClass, integraseFamily });
