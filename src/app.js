...
function featureSummary(r) {
  ...
  const cls = r.hostFeatureClass ? `; ${r.hostFeatureClass}` : '';
  const reason = r.candidateReason ? `; ${r.candidateReason}` : '';
  const dist = r.distanceToFeatureBp === null || r.distanceToFeatureBp === undefined ? 'NA' : `${r.distanceToFeatureBp} bp`;
  if (!type && !label) return `${prox}${cls}${reason}`;
  return `${prox}; ${type || 'feature'} ${label ? '(' + label + ')' : ''}; distance ${dist}${cls}${reason}`;
}

function matchSummary(r) {
  const model = r.matchModel || 'exact_core';
  const family = r.integraseFamily ? `integrase: ${r.integraseFamily}` : '';
  const source = r.integraseFamilySource ? `source: ${r.integraseFamilySource}` : '';
  const fit = r.modelFit ? `fit: ${r.modelFit}` : '';
  const reason = r.modelFitReason ? r.modelFitReason : '';
  const evidence = r.serineEvidence || '';
  return [model, family, source, fit, reason, evidence].filter(Boolean).join('; ');
}
...
function toTSV(rows) {
  const cols = [
    ...
    'integrase','integraseFamily','integraseFamilyInferred','integraseFamilySource','modelFit','modelFitReason','integraseCoords','queryMode','matchModel','serineEvidence','searchType','searchSources','duplicateCount',
    'hostLocus','strand','product','hostFeatureClass','candidateReason','nearestFeatureType','nearestFeatureLabel','nearestFeatureCoords',
...
function toFasta(rows) {
  return rows.map((r, i) => {
    ...
    const fields = [
      ...
      `integraseFamily=${fastaValue(r.integraseFamily)}`,
      `integraseFamilySource=${fastaValue(r.integraseFamilySource)}`,
      `modelFit=${fastaValue(r.modelFit)}`,
      `matchModel=${fastaValue(r.matchModel)}`,
      ...
      `hostFeatureClass=${fastaValue(r.hostFeatureClass)}`,
...
function collectSettings() {
  return {
    ...
    serineHostSearch: $('serineHostSearch').checked,
    serineStemSearch: $('serineStemSearch').checked,
    serineCoreMin: Number($('serineCoreMin').value || 8),
    serineStemMatches: Number($('serineStemMatches').value || 5),
    integraseFamilyMode: $('integraseFamilyMode').value || 'auto',
    filterByIntegraseType: $('filterByIntegraseType').checked,
    showOffModelCandidates: $('showOffModelCandidates').checked,
    offModelExactCoreMin: Number($('offModelExactCoreMin').value || 26)
  };
}
...
function reverseComplement(seq) {
  ...
  return String(seq || '').toUpperCase().split('').reverse().map(b => map[b] || 'N').join('');
}
function makeStemLikeContext(leftArm, core, leftPadSeed, rightPadSeed) {
  const left = String(leftArm).toUpperCase();
  const middle = String(core).toUpperCase();
  const right = reverseComplement(left);
  return `${demoPad(leftPadSeed, 14)}${left}${middle}${right}${demoPad(rightPadSeed, 14)}`;
}
function loadDemo() {
  /*
    This demo is intentionally synthetic. It does not contain any experimental
    attP/attB sequence supplied by users and is not read by the prediction
    worker. It only creates toy files so the UI can be tried without uploads.
  */
  const demoCore = 'ATGCTAGC';
  const hostCtx = makeStemLikeContext('TACGTA', demoCore, 31, 32);
  const phageCtx = makeStemLikeContext('CGTACG', demoCore, 33, 34);
  const phage = `LOCUS       DemoSerinePhage  5000 bp    DNA     linear   PHG 01-JAN-2026
FEATURES             Location/Qualifiers
     CDS             1900..2450
                     /gene="int"
                     /product="large serine integrase"
ORIGIN
        1 ${demoPad(11, 2060)}${phageCtx}${demoPad(12, 2877)}
//`;
  const hostIndex = {
    schema: 'att-site-host-index-v1',
    created_utc: new Date().toISOString(),
    metadata: { name: 'DemoHost host_index.json', organism: 'Demo host' },
    parameters: { flank: 500, feature_types: ['tRNA','tmRNA','rRNA'], serine_cds_hotspots: true, global_chunks_included: false },
    ...
    feature_neighborhoods: [{
      id: 'DemoHost:CDS:3900-5600:1',
      contig_id: 'DemoHost',
      feature_type: 'CDS',
      feature_label: 'groL / GroEL chaperonin 60',
      feature_start: 3900,
      feature_end: 5600,
      feature_strand: 1,
      candidate_reason: 'serine-integrase CDS hotspot: groL/groEL/chaperonin 60',
      window_start_approx: 3400,
      window_end_approx: 6100,
      window_is_circular_wrapped: false,
      sequence: `${demoPad(21, 620)}${hostCtx}${demoPad(22, 2023)}`,
      length: 2700
    }],
    global_chunks: []
  };
  state.phageFiles = [makeFile('demo_serine_phage.gbk', phage)];
  state.bactFiles = [makeFile('demo_host_index.json', JSON.stringify(hostIndex))];
  $('integraseFamilyMode').value = 'serine';
  renderFiles(); resetLog('Synthetic serine-integrase groL/groEL small-core/stem demo loaded. The integrase type setting was set to known serine. Click Run attP/attB prediction.');
}
